...
Each Go+CUE package has a `_test.cue` file (e.g. `sprig/sprig_test.cue`,
`text/template/template_test.cue`) that defines `@test` versions of the
inject bindings plus test expressions. This allows the Go code to be tested
in CI before its pseudo-version is published.
